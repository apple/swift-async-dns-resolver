...
[[ -d swift-async-dns-resolver.xcodeproj ]] || swift package generate-xcodeproj
...
jazzy_args=(--clean
            --author 'SwiftAsyncDNSResolver team'
            --readme "$module_switcher"
            --author_url https://github.com/apple/swift-async-dns-resolver
            --github_url https://github.com/apple/swift-async-dns-resolver
            --github-file-prefix https://github.com/apple/swift-async-dns-resolver/tree/$version
            --theme fullwidth
            --xcodebuild-arguments -scheme,swift-async-dns-resolver-Package)
cat > "$module_switcher" <<"EOF"
# SwiftAsyncDNSResolver Docs

SwiftAsyncDNSResolver is a Swift package for DNS resolutions.

To get started with SwiftAsyncDNSResolver, [`import AsyncDNSResolver`](../AsyncDNSResolver/index.html). The
most important type is [`AsyncDNSResolver`](https://apple.github.io/swift-async-dns-resolver/docs/current/AsyncDNSResolver/Classes/AsyncDNSResolver.html)
which you can use for DNS queries.
EOF
...
for module in "${modules[@]}"; do
  args=("${jazzy_args[@]}" --output "$jazzy_dir/docs/$version/$module" --docset-path "$jazzy_dir/docset/$version/$module"
        ...
        --root-url "https://apple.github.io/swift-async-dns-resolver/docs/$version/$module/")
  if [[ -f "$root_path/.build/sourcekitten/$module.json" ]]; then
...
if [[ $PUSH == true ]]; then
  ...
  git add --all docs
  echo '<html><head><meta http-equiv="refresh" content="0; url=docs/current/AsyncDNSResolver/index.html" /></head></html>' > index.html
  git add index.html
  touch .nojekyll
  git add .nojekyll
